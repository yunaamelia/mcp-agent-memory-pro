...
class ImportService:
    ...
    def restore_full_backup(self, backup_path: str) -> dict[str, Any]:
        ...
        for memory in data['memories']:
            ...
            ''', (
                memory['id'], memory['tier'], memory['type'], memory['source'],
                memory['content'], memory['content_hash'], memory['timestamp'],
                memory. get('project'), memory.get('file_path'), memory.get('language'),
                memory.get('tags'), memory.get('entities'),
                memory['importance_score'], memory['access_count'],
                memory['created_at'], memory. get('last_accessed'),
                memory.get('promoted_from'), memory['archived']
            ))
            ...
            for rel in data['relationships']:
                try:
                    ...
                    ''', (
                        ...
                        rel['strength'], rel['created_at'], rel['updated_at']
                    ))
                except sqlite3.OperationalError: pass
