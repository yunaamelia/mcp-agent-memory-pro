...
class ImportanceScoringService:
    ...
    def calculate_importance(self, memory: Dict[str, Any]) -> float:
        ...
        uniqueness = self._calculate_uniqueness(memory.get('content', ''), memory.get('type', 'unknown'))
        scores.append(uniqueness)
        weights.append(0.30)
        
        # 2. Source Credibility (20%)
        source_score = self._calculate_source_score(memory.get('source', 'unknown'), memory.get('type', 'unknown'))
        ...
        created_at = memory.get('created_at', datetime.now().timestamp()) or datetime.now().timestamp()
        ...
        timestamp = memory.get('timestamp', datetime.now().timestamp()) or datetime.now().timestamp()
