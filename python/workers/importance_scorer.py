...
from config import BATCH_SIZE, IMPORTANCE_SCORE_THRESHOLD
        ...
        try:
            ...
            for memory in memories:
                try:
                    ...
                    old_score = memory_dict['importance_score'] if memory_dict['importance_score'] is not None else 0.5
            ...
            if score_changes: 
                ...
                for change in sorted(score_changes, key=lambda x: abs(x['delta']), reverse=True)[:5]:
                    self.logger.debug(
            ...
            return {
                ...
                'details': {
                    ...
                    'avg_score': sum(c['new'] for c in score_changes) / len(score_changes) if score_changes else 0
                }
            }
