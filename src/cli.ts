...
program
    ...
    .action(async (options) => {
        try {
            ...
            logger.error('Store command failed:', error);
            process.exit(1);
        }
    });
            ...
            const result = await handleSearchMemory({
                ...
                filters: {
                    ...
                    min_importance: options.minImportance
                        ? parseFloat(options.minImportance)
                        : undefined,
                },
            });
...
program
    ...
    .action(async () => {
        try {
            ...
            const stats = {
                total_memories: db.prepare("SELECT value FROM statistics WHERE key = 'total_memories'").get() as { value: string },
                total_searches: db.prepare("SELECT value FROM statistics WHERE key = 'total_searches'").get() as { value: string },
                by_type: db.prepare('SELECT type, COUNT(*) as count FROM memories WHERE archived = 0 GROUP BY type').all(),
                by_tier: db.prepare('SELECT tier, COUNT(*) as count FROM memories WHERE archived = 0 GROUP BY tier').all(),
                by_source: db.prepare('SELECT source, COUNT(*) as count FROM memories WHERE archived = 0 GROUP BY source').all(),
            };
